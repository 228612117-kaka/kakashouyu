...
const App: React.FC = () => {
  ...
  const handleSearch = useCallback(async (e?: React.FormEvent, targetWord?: string) => {
    ...
    try {
      ...
      setResult(details);
      const img = await generateSignImage(details.word, details.movement);
      setSignImage(img);
      setState(AppState.SUCCESS);
    } catch (err) {
      console.error(err);
    ...
    <>
      <section className="text-center mb-12">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          输入您想了解的词语，手语通将为您提供详细的手势解析与视觉参考。
        </p>
      </section>
              ...
              {signImage ? (
                <div className="relative group w-full flex flex-col items-center">
                  <img 
                    src={signImage} 
                    alt={result.word} 
                    className="w-full max-w-sm rounded-2xl shadow-lg border-4 border-white"
                  />
                  {isEditing && (
                ...
                <div className="w-full max-w-sm">
                  {isEditing ? (
                    <label className="cursor-pointer bg-white border-2 border-dashed border-gray-300 rounded-2xl p-12 flex flex-col items-center hover:border-blue-400 hover:bg-blue-50 transition-all">
                        ...
                        <span className="text-sm font-medium text-gray-500">上传手势演示图</span>
                        <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                    </label>
                  ) : (
                    <div className="flex flex-col items-center text-gray-400 gap-4">
                      <div className="animate-pulse bg-gray-200 h-64 w-64 rounded-2xl"></div>
                      <p className="text-sm">正在获取 AI 视觉参考图...</p>
                    </div>
                  )}
                </div>
              )}
              {!isEditing && signImage && (
                <div className="mt-4 text-center">
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    {storageService.getWord(result.word) ? '本地自定义版本' : 'AI 智能生成'}
                  </span>
                </div>
              )}
            </div>
